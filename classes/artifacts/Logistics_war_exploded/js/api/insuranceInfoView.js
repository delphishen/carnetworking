Ext.namespace('Ext.insuranceInfo');

var flag = '1';

Ext.insuranceInfo.form = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        ...
        Ext.insuranceInfo.form.superclass.constructor.call(this, {
            labelWidth: 60,
            baseCls: 'x-plain',
            layout: 'column',
            style: 'padding : 5',
...
Ext.insuranceInfo.win = Ext.extend(Ext.Window, {
    constructor: function (app) {
        this.app = app;
        this.form = new Ext.insuranceInfo.form(this);
        Ext.insuranceInfo.win.superclass.constructor.call(this, {
            width: 500,
            plain: true,
            showLock: true,
            modal: true,
    ...
    onSave: function (btn) {
        ...
        if (form.isValid()) {
            ...
            var insuranceInfo = form.getValues();
            console.log(insuranceInfo);
            /*		insuranceInfo.isAdmin = insuranceInfo.isAdmin == 'true' ? 1 : 0;*/
            Ext.eu.ajax(path + '/api/saveInsuranceInfo.do', {
                insuranceInfo: Ext.encode(insuranceInfo)
            }, function (resp) {
...
Ext.insuranceInfo.grid = Ext.extend(Ext.grid.GridPanel, {
    constructor: function (app) {
        this.app = app;
        // 数据源
        this.ds = new Ext.data.JsonStore({
            url: path + '/api/queryInsuranceInfo.do',
            idProperty: 'id',
            root: 'rows',
            totalProperty: 'results',
            fields: ['id', 'insuranceName', 'chargePerson', 'contactNumber', 'scale',
                'fleetId', 'fleetName', 'company', 'address', 'remark', 'orgnId','startDate','endDate'],
            autoDestroy: true,
            autoLoad: true,
            baseParams: {
                isPaging: true,
        ...
        // 构造
        Ext.insuranceInfo.grid.superclass.constructor.call(this, {
            region: 'center',
            loadMask: 'loading...',
            columnLines: true,
            clicksToEdit: 1,
    ...
    onAdd: function (btn) {
        flag = '1';
        var win = new Ext.insuranceInfo.win(this);
        win.setTitle('添加保险公司信息', 'add');
        win.show();
    },
    onModify: function (btn) {
        ...
        form.findField('startDate').setValue(select.startDate);
        form.findField('endDate').setValue(select.endDate);
        win.show();
    },
    onDelete: function () {
        var selects = Ext.eu.getSelects(this);
        ...
        for (var i = 0; i < selects.length; i++) {
            ...
            employees.push(insuranceInfo);
        }
        Ext.Msg.confirm('删除操作', '确定要删除所选记录吗?', function (btn) {
            if (btn == 'yes') {
                Ext.eu.ajax(path + '/api/deleteInsuranceInfo.do',
                    {
                        insuranceInfos: Ext.encode(employees)
                    }, function (resp) {
                        var res = Ext.decode(resp.responseText);
...
Ext.insuranceInfo.queryPanel = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        this.app = app;
        ...
        // panel定义
        Ext.insuranceInfo.queryPanel.superclass.constructor.call(this, {
            region: 'north',
            height: 40,
            frame: true,
            split: true,
 ...
 * @return {}
 */
var insuranceInfoView = function (params) {
    this.queryPanel = new Ext.insuranceInfo.queryPanel(this);
    this.grid = new Ext.insuranceInfo.grid(this);
