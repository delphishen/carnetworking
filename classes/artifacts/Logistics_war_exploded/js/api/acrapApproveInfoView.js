Ext.namespace('Ext.acrapApproveInfo');

var flag = '1';

Ext.acrapApproveInfo.form = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        ...
        Ext.acrapApproveInfo.form.superclass.constructor.call(this, {
            labelWidth: 60,
            baseCls: 'x-plain',
            layout: 'column',
            style: 'padding : 5',
...
Ext.acrapApproveInfo.win = Ext.extend(Ext.Window, {
    constructor: function (app) {
        this.app = app;
        this.form = new Ext.acrapApproveInfo.form(this);
        Ext.acrapApproveInfo.win.superclass.constructor.call(this, {
            width: 500,
            plain: true,
            showLock: true,
            modal: true,
    ...
    onSave: function (btn) {
        ...
        if (form.isValid()) {
            ...
            var acrapApproveInfo = form.getValues();
            console.log(acrapApproveInfo);
            /*		acrapApproveInfo.isAdmin = acrapApproveInfo.isAdmin == 'true' ? 1 : 0;*/
            Ext.eu.ajax(path + '/api/saveAcrapApproveInfo.do', {
                acrapApproveInfo: Ext.encode(acrapApproveInfo)
            }, function (resp) {
...
Ext.acrapApproveInfo.grid = Ext.extend(Ext.grid.GridPanel, {
    constructor: function (app) {
        this.app = app;
        // 数据源
        this.ds = new Ext.data.JsonStore({
            url: path + '/api/queryAcrapApproveInfo.do',
            idProperty: 'id',
            root: 'rows',
            totalProperty: 'results',
            fields: ['id', 'scrapDate', 'scrapVehicle', 'brandName', 'vehicleModel', 'displacement', 'engineNo', 'registrationDate', 'scrapYears', 'reason',
                'fleetId', 'fleetName', 'company', 'identificationNo', 'useYears', 'orgnId','kilometers','approveOrgn', 'approvePerson', 'approveDate','approveStatus','approveDesc'],
            autoDestroy: true,
            autoLoad: true,
            baseParams: {
                isPaging: true,
        ...
        // 构造
        Ext.acrapApproveInfo.grid.superclass.constructor.call(this, {
            region: 'center',
            loadMask: 'loading...',
            columnLines: true,
            clicksToEdit: 1,
    ...
    onAdd: function (btn) {
        flag = '1';
        var win = new Ext.acrapApproveInfo.win(this);
        win.setTitle('添加车辆报废审批信息', 'add');
        win.show();
    },
    onModify: function (btn) {
        ...
        win.setTitle('修改车辆报废审批信息', 'modify');
        form.findField('id').setValue(select.id);
        form.findField('scrapDate').setValue(select.scrapDate);
        form.findField('scrapVehicle').setValue(select.scrapVehicle);
    ...
    onDelete: function () {
        ...
        for (var i = 0; i < selects.length; i++) {
            var acrapApproveInfo = {
                id: selects[i].data.id
            }
            employees.push(acrapApproveInfo);
        }
        Ext.Msg.confirm('删除操作', '确定要删除所选记录吗?', function (btn) {
            if (btn == 'yes') {
                Ext.eu.ajax(path + '/api/deleteAcrapApproveInfo.do',
                    {
                        acrapApproveInfos: Ext.encode(employees)
                    }, function (resp) {
                        var res = Ext.decode(resp.responseText);
...
Ext.acrapApproveInfo.queryPanel = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        this.app = app;
        ...
        // panel定义
        Ext.acrapApproveInfo.queryPanel.superclass.constructor.call(this, {
            region: 'north',
            height: 40,
            frame: true,
            split: true,
 ...
 * @return {}
 */
var acrapApproveInfoView = function (params) {
    this.queryPanel = new Ext.acrapApproveInfo.queryPanel(this);
    this.grid = new Ext.acrapApproveInfo.grid(this);
