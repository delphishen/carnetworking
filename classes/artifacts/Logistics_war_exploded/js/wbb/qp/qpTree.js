...
Ext.qpTree.win = Ext.extend(Ext.Window, {
			constructor : function(app) {
				this.app = app;
				this.form = new Ext.qpTree.form(this);
				Ext.qpTree.win.superclass.constructor.call(this, {
							width : 500,
							plain : true,
							showLock : true,
							modal : true,
			...
			onSave : function(btn) {
				...
				if (form.isValid()) {
					...
					Ext.eu.ajax(path + '/wbb/saveQpSort.do', {
								qpSort : Ext.encode(form.getValues())
							}, function(resp) {
...
Ext.qpTree.tree = Ext.extend(Ext.tree.TreePanel, {
	constructor : function(app) {
		...
		Ext.qpTree.tree.superclass.constructor.call(this, {
					...
					loader : new Ext.tree.TreeLoader({
								dataUrl : path + '/wbb/buildTreeQpSort.do',
								baseParams : {}
							}),
					listeners : {
						contextmenu : {
						...
						click : function(node, event) {
							if (node.id == 0) {
								Ext.getCmp('addQp').disable();
							} else {
								Ext.getCmp('addQp').enable();
							}
	...
	onAdd : function() {
		...
		var win = new Ext.qpTree.win(this);
	...
	onDelete : function() {
		...
		Ext.eu.ajax(path + '/wbb/queryQp.do', {
					qpSortId : '\'' + node.id + '\''
				}, function(resp) {
