Ext.namespace('Ext.oilStationInfo');

var flag = '1';

Ext.oilStationInfo.form = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        ...
        Ext.oilStationInfo.form.superclass.constructor.call(this, {
            labelWidth: 60,
            baseCls: 'x-plain',
            layout: 'column',
            style: 'padding : 5',
...
Ext.oilStationInfo.win = Ext.extend(Ext.Window, {
    constructor: function (app) {
        this.app = app;
        this.form = new Ext.oilStationInfo.form(this);
        Ext.oilStationInfo.win.superclass.constructor.call(this, {
            width: 500,
            plain: true,
            showLock: true,
            modal: true,
    ...
    onSave: function (btn) {
        ...
        if (form.isValid()) {
            ...
            var oilStationInfo = form.getValues();
            console.log(oilStationInfo);
            /*		oilStationInfo.isAdmin = oilStationInfo.isAdmin == 'true' ? 1 : 0;*/
            Ext.eu.ajax(path + '/api/saveOilStationInfo.do', {
                oilStationInfo: Ext.encode(oilStationInfo)
            }, function (resp) {
...
Ext.oilStationInfo.grid = Ext.extend(Ext.grid.GridPanel, {
    constructor: function (app) {
        this.app = app;
        // 数据源
        this.ds = new Ext.data.JsonStore({
            url: path + '/api/queryOilStationInfo.do',
            idProperty: 'id',
            root: 'rows',
            totalProperty: 'results',
            fields: ['id', 'oilStationName', 'chargePerson', 'contactNumber', 'scale',
                'fleetId', 'fleetName', 'company', 'address', 'remark', 'orgnId','lon','lat'],
            autoDestroy: true,
            autoLoad: true,
            baseParams: {
                isPaging: true,
        ...
        // 构造
        Ext.oilStationInfo.grid.superclass.constructor.call(this, {
            region: 'center',
            loadMask: 'loading...',
            columnLines: true,
            clicksToEdit: 1,
    ...
    onAdd: function (btn) {
        flag = '1';
        var win = new Ext.oilStationInfo.win(this);
        win.setTitle('添加加油站点信息', 'add');
        win.show();
    },
    onModify: function (btn) {
        ...
        win.setTitle('修改加油站点信息', 'modify');
        form.findField('id').setValue(select.id);
        form.findField('oilStationName').setValue(select.oilStationName);
        ...
        form.findField('lon').setValue(select.lon);
    ...
    onDelete: function () {
        ...
        for (var i = 0; i < selects.length; i++) {
            ...
            employees.push(oilStationInfo);
        }
        Ext.Msg.confirm('删除操作', '确定要删除所选记录吗?', function (btn) {
            if (btn == 'yes') {
                Ext.eu.ajax(path + '/api/deleteOilStationInfo.do',
                    {
                        oilStationInfos: Ext.encode(employees)
                    }, function (resp) {
                        var res = Ext.decode(resp.responseText);
...
Ext.oilStationInfo.queryPanel = Ext.extend(Ext.FormPanel, {
    constructor: function (app) {
        this.app = app;
        ...
        // panel定义
        Ext.oilStationInfo.queryPanel.superclass.constructor.call(this, {
            region: 'north',
            height: 40,
            frame: true,
            split: true,
 ...
 * @return {}
 */
var oilStationInfoView = function (params) {
    this.queryPanel = new Ext.oilStationInfo.queryPanel(this);
    this.grid = new Ext.oilStationInfo.grid(this);
