...
Ext.extend(Ext.ensible.cal.MonthViewTemplate, Ext.XTemplate, {
     ...
     * The date format to use for the day header's HTML title attribute displayed on mouseover 鼠标悬停
                ...
                for (; dayIndex < dayCount; dayIndex++) {
                    ...
                    if (weekGrid && weekGrid[dayIndex]) {
                        ...
                        if (skippedEventCount > 0) {
                          ...
                          //  alert( String.format(config.getMoreText(skippedEventCount), skippedEventCount));
                         //   alert('weekIndex='+weekIndex+'  ,maxEventsPerDay='+maxEventsPerDay);
                        }
                        ...
                        //ssm 2017.08.10  增加这个，解决日历控件点击取不到正确日期问题
                      //  currentDateString = currentDate.format('Ymd');
                        currentDateString = currentDate.format('Ymd');
...
Ext.ensible.cal.CalendarView = Ext.extend(Ext.BoxComponent, {
     ...
     */
    enableDD: false,  //鼠标拖拽关闭 2017.08.10
    /**
     ...
     */
    enableContextMenus: false,  //鼠标右键菜单关闭 2017.08.10
    /**
    ...
    onClick : function(e, t){
        ...
        if(el){
            ...
            if(this.fireEvent('eventclick', this, rec, el) !== false){
            	// ssm 点击内容触发      alert(Ext.encode(rec));
...
//        data.Title = (evt[M.IsAllDay.name] ? '' : evt[M.StartDate.name].format(fmt)) + (!title || title.length == 0 ? this.defaultEventTitleText : title);
    ...
    onClick : function(e, t){
        ...
        if(Ext.ensible.cal.MonthView.superclass.onClick.apply(this, arguments)){
            ...
            //ssm 点击有内容的   ,这里原本是没代码的,增加这两行是解决 点击日历格有内容的信息时弹出的窗口获取日期错误
           //修复这个bug 花掉我1天的时间。
