...
//					.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
	...
	uploadFile : function(record) {
		...
		Ext.apply(params,{category:this.category});
		//ssm  增加参数20170813
		Ext.apply(params,{link:this.link});
	...
	updateRecordState : function(data) {
		if ("success" in data.response && data.response.success) {
			...
			data.record.set("note", '上传文件成功.'+data.response.message
							|| data.response.error
	...
	onCloseButtonClick : function() {
		...
		if(this.callback!=null){
			this.callback.call(this,fileArr,this.scope?this.scope:{});
		}
		this.close();
