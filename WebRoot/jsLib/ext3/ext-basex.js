 ...
 *  - Adds Browser capabilities object reporting on presence of (SVG, Canvas,
 * Flash, Cookies, XPath ) if(Ext.capabilities.hasFlash){ ... } - Adds
 ...
 * Array.slice support for other browsers (Gecko already supports it) @example:
 * Array.slice( someArray, 2 ) - Adds Ext[isFunction, isObject, isDocument,
 * isElement, isEvent] methods. - Adds Ext.isEventSupported('resize'[,
 * forElement]) to determine if the browser supports a specific event. - Adds
 * multiPart Response handling (via onpart callbacks and/or parts Array of
 * response Object) - Adds parsed contentType to response objects - Adds Xdomain
 * request support for modern browsers
 * 
 * ***********************************************************************************
...
(function() {
	...
	Ext.extend(Ext.lib.Ajax.Queue, Object, {
				...
				/**
				 * @property {Boolean} suspended Indicate the suspense state of
				 *           the queue.
				 */
				suspended : false,
				/**
				 * @property {Object} activeRequest A reference to current/last
				 *           active request.
				 */
				activeRequest : null,
				 ...
				 * 
				 * @param {Boolean}
				 *            peek If true, the queue item is returned but not
				 * removed from the stack. @ default false
				 */
				next : function(peek) {
					var req = peek
				...
				/**
				 * Suspend queue further queue dispatches of any remaining
				 * (pending) requests until the
				 * {@link #Ext.ux.ModuleManager-resume} method is called.
				 */
				suspend : function() {
					this.suspended = true;
				},
				...
				/**
				 * Dispatches the next queue item and initiates a Ext.lib.Ajax
	...
	Ext.apply(A, {
		...
		headers : A.headers || {},
		defaultPostHeader : A.defaultPostHeader
		 ...
		 *           concurrent Queued browser (XHR) requests Note: IE8
		 *           increases this limit to 6
		 */
		maxConcurrentRequests : Ext.isIE ? Ext.value(
				window.maxConnectionsPerServer, 2) : 4,
		...
		createXhrObject : function(transactionId, options) {
			...
			options || (options = {});
			try {
				options.xdomain && window.XDomainRequest
						&& (obj.conn = new XDomainRequest());

				if (!defined(obj.conn) && window.ActiveXObject
				...
				var actX = window.ActiveXObject ? (options.multiPart
						? this.activeXMultipart
						: this.activeX) : null;
		...
		serializeForm : function() {
			var reSelect = /select-(one|multiple)/i, reInput = /file|undefined|reset|button/i, reChecks = /radio|checkbox/i;

			return function(form) {
				var fElements = form.elements
						|| (document.forms[form] || Ext.getDom(form)).elements, hasSubmit = false, encoder = this.encoder, element, options, name, val, data = '', type;
				forEach(fElements, function(element) {
					...
					if (!element.disabled && name) {
						if (reSelect.test(type)) {
							...
							if (!(reChecks.test(type) && !element.checked)
				...
				if (o.status.isOK && !o.status.isError) {
					if (!this.events
						...
						Ext.isFunction(callback[cb])
								&& callback[cb].call(callback.scope || null,
										responseObject);
					...
					if (!this.events
							|| this.fireEvent('exception', o, responseObject,
									callback, isAbort, isTimeout,
									responseObject.fullStatus.error) !== false) {
						Ext.isFunction(callback.failure)
								&& callback.failure.call(
										callback.scope || null, responseObject,
										responseObject.fullStatus.error);
		...
		createResponseObject : function(o, callbackArg, isAbort, isTimeout) {
			...
			if (isAbort !== true) {
				...
				try {
					headerStr = ('getAllResponseHeaders' in o.conn ? o.conn
					...
					headerStr.split('\n').forEach(function(sHeader) {
						(s = sHeader.split(':'))
								&& s.first()
								&& (headerObj[s.first().trim().toLowerCase()] = (s
										.last() || '').trim());
					});
				...
				if ((o.status.isLocal || o.proxied)
					...
					if (o.status.isOK
							&& ((!obj.responseXML && this.reCtypeXML
									.test(obj.contentType)) || (obj.responseXML && obj.responseXML.childNodes.length === 0))) {
					...
					if (obj.responseXML) {
						...
						parseBad
								|| (obj.contentType = headerObj['content-type'] = obj.responseXML.contentType
										|| 'text\/xml');
					}
				}
		...
		request : function(method, uri, cb, data, options) {
			...
			if (!this.events
				...
				options.onpart
						&& (cb.onpart || (cb.onpart = Ext
								.isFunction(options.onpart) ? options.onpart
								.createDelegate(options.scope) : null));
			...
			try {
				if (f = options.proxied) { /* JSONP scriptTag Support */
					...
					window[o.cbName] = o.cb = function(content, request) {
						...
						Ext.isFunction(this.onreadystatechange)
		...
		onStateChange : function(o, callback, mode) {
			...
			if (mode === 'load' || readyState > 2) {
				...
				if (ct && /multipart\//i.test(ct)) {
					var r = null, boundary = ct.split('"')[1], kb = '--'
							+ boundary;
					o.multiPart = true;
					try {
						r = C.responseText;
			...
			(readyState === 4 || mode === 'load')
					&& A.handleTransactionResponse(o, callback);
			this.events
					&& this.fireEvent.apply(this, ['readystatechange']
									.concat(Array.slice(arguments, 0)));
		},
	...
	var monitoredNode = function(tag, attributes, callback, context, deferred) {
		...
		if (doc && head && (node = Ext.get(doc.createElement(tag)))) {
			...
			ndom && forEach(attributes, function(value, attrib) {
						value && (attrib in ndom)
								&& ndom.setAttribute(attrib, value);
					});

			if (callback) {
				var cb = (callback.success || callback).createDelegate(
						callback.scope || null, [callback], true);
				Ext.capabilities.isEventSupported('load', tag) ? node.on(
						"load", cb) : cb.defer(50);
			}
	...
	Ext.applyIf(Array.prototype, {
				...
				/*
				 * Fix for IE, Opera < 9.5, which does not seem to include the
				 * map function on Array's
				 */
				...
				/**
				 * Return true of the passed Function test true of ANY array
				 * elememt. (added for IE)
				 */
				some : function(fn) {
				...
				/**
				 * Return true of the passed Function test true of ALL array
				 * elememts. (added for IE)
				 */
				every : function(fn) {
					var f = Ext.isFunction(fn) ? fn : function() {
					};
	...
	Ext.clone = function(obj, deep) {
		...
		forEach(obj, function(value, name, objAll) {
					o[name] = (value === objAll ? // reference to itself?
							o
							: deep ? Ext.clone(value, true) : value);
				}, obj, deep);
		return o;
	};
	...
	Ext.applyIf(Array, {
				// Permits: Array.slice(arguments, 1); // mozilla already
				// supports this
				slice : function(obj) {
	...
	Ext.applyIf(Ext, {
		...
		isIterable : function(obj) {
			...
			return !!((/NodeList|HTMLCollection/i).test(OP.toString.call(obj))
		...
		isDocument : function(obj) {
			return OP.toString.apply(obj) == '[object HTMLDocument]'
					|| (obj && obj.nodeType === 9);
		},
		...
		isEvent : function(obj) {
			return OP.toString.apply(obj) == '[object Event]'
					|| (Ext.isObject(obj) && !Ext.type(obj.constructor) && (window.event
							&& obj.clientX && obj.clientX === window.event.clientX));
		},
	...
	Ext.capabilities = {
		/**
		 * @property {Boolean} hasActiveX True if the Browser support (and is
		 *           enabled) ActiveX.
		 */
		hasActiveX : !!window.ActiveXObject,

		/**
		 * @property {Boolean} hasXDR True, if the Browser has Cross-Domain Ajax
		 *           request capability.
		 */
		hasXDR : function() {
		...
		hasFlash : (function() {
			...
			if (window.ActiveXObject) {
				...
				for (var i = 0, length = navigator.plugins.length; i < length; i++) {
					// test to see if any plugin names contain the word flash,
					// if so it must support it - return true
		...
		/**
		 * @property {Boolean} hasCanvas True if the browser has canvas Element
		 *           support.
		 */
		 ...
		 */
		hasSVG : !!(document.createElementNS && document.createElementNS(
				'http://www.w3.org/2000/svg', 'svg').width),
		...
		isEventSupported : function(evName, testEl) {
			...
			getKey = function(type, el) {
				...
				return (tEl ? (Ext.isElement(tEl) || Ext.isDocument(tEl)
						? tEl.nodeName.toLowerCase()
						: el.self ? '#window' : el || '#object') : el || 'div')
						+ ':' + type;
			};
			...
			return function(evName, testEl) {
				...
				isSupported
						|| (isSupported = window.Event
								&& !!(String(evName).toUpperCase() in window.Event));
				...
				if (object) {
					...
					for (key in object) {
						(!!protos || object.hasOwnProperty(key))
								&& block.call(context || object, object[key],
										key, object);
					}
				}
			},
